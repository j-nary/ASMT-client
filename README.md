...
`  `팀원 – 컴퓨터학부 20211405 김은수   팀원 – 컴퓨터학부 20212895 김황조   팀원 – 컴퓨터학부 20211442 정종우
...
1) 백엔드 개발: MySQL 데이터베이스와의 통신을 위해 JPA 를 활용하 고, RESTful API 를 설계하여 클라이언트와 서버 간의 데이터 통신 을 처리한다. 데이터베이스에는 음식점, 북마크, 랭킹 정보를 저장하 고 동적으로 쿼리를 구성하여 필터링된 데이터를 제공한다. 
1) 데이터 관리: 음식점 데이터를 얻기 위해 네이버 지도 데이터를 크롤 링하여 수집하였다. 크롤링 과정에서 robots.txt 규약을 지키고, 필요 한 데이터 형식에 맞게 재가공하여 사용한다. 
